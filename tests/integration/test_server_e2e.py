...
def start_server() -> subprocess.Popen[str]:
    ...
    # Write uvicorn output to log file for debugging
    log_file = open("uvicorn_e2e.log", "w", encoding="utf-8")
    
    def log_output_to_file(pipe: Any, log_func: Any) -> None:
        for line in iter(pipe.readline, ""):
            log_func(line.strip())
            log_file.write(line)
            log_file.flush()
    ...
    threading.Thread(
        target=log_output_to_file, args=(process.stdout, logger.info), daemon=True
    ).start()
    threading.Thread(
        target=log_output_to_file, args=(process.stderr, logger.error), daemon=True
    ).start()
