...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    # Set dummy GCP project/location to bypass google auth metadata checks on local systems
    import os
    import google.auth
    class MockCredentials:
        def before_request(self, *args, **kwargs):
            pass
    google.auth.default = lambda *args, **kwargs: (MockCredentials(), "dummy-project")
    ...
    def get_runtime() -> AdkApp:
        ...
        if runtime is None:
            ...
            runtime.set_up()
            # Restore local execution environment variables if GCP credentials are absent
            import os
            if os.environ.get("GOOGLE_API_KEY"):
                os.environ["GOOGLE_GENAI_USE_VERTEXAI"] = "False"
                os.environ["GOOGLE_GENAI_USE_ENTERPRISE"] = "False"
                # Re-instantiate runners to use direct Gemini models
                from google.adk.runners import Runner
                runtime._tmpl_attrs["runner"] = Runner(
                    app=runtime._tmpl_attrs.get("app"),
                    session_service=runtime._tmpl_attrs.get("session_service"),
                    artifact_service=runtime._tmpl_attrs.get("artifact_service"),
                    memory_service=runtime._tmpl_attrs.get("memory_service"),
                )
                runtime._tmpl_attrs["in_memory_runner"] = Runner(
                    app=runtime._tmpl_attrs.get("app"),
                    session_service=runtime._tmpl_attrs.get("in_memory_session_service"),
                    artifact_service=runtime._tmpl_attrs.get("in_memory_artifact_service"),
                    memory_service=runtime._tmpl_attrs.get("in_memory_memory_service"),
                )
